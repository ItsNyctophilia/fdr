...
static void probe_client(const struct sockaddr *client,
	...
	if (client->sa_family == AF_INET6) {
		...
		inet_ntop(client->sa_family,
			  &((struct sockaddr_in *)client)->sin_addr, info->addr,
			  sizeof(info->addr));
...
static void serve_port(int sd) {
	...
	for (;;) {
		...
		case 'F':
			...
			err =
			    fib_to_hex(input + 1, response, BUF_LEN, uppercase);
			if (err) {
		...
		sendto(sd, response, strlen(response), 0,
		       (struct sockaddr *)&client, client_sz);
		log_response((const struct sockaddr *)&client, sd, input,
			     response);
	}
}
